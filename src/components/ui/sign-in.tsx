...
const AnimatedPhrases = () => {
  const phrases = ["Conecte-se e transforme sua comunicação com facilidade e eficiência", "Desperte o poder da comunicação inteligente em suas mãos", "Sua mensagem, nossa tecnologia. Resultados extraordinários", "Conecte corações, conquiste mentes, transforme negócios", "A revolução da comunicação começa com você"];
  ...
  return <div className="h-12 flex items-center justify-center">
      <p className={`text-muted-foreground text-center transition-all duration-300 ${isVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-2'}`}>
...
export const SignInPage: React.FC<SignInPageProps> = ({
  ...
  return <div className="h-[100dvh] flex flex-col md:flex-row font-geist w-[100dvw]">
      {/* Left column: sign-in form */}
      <section className="w-full md:flex-1 flex items-center justify-center p-4 md:p-8 py-6 md:py-8 min-h-screen md:min-h-0">
          ...
          <div className="flex flex-col gap-4">
            ...
            <form className="space-y-5 w-full" onSubmit={onSignIn}>
              ...
              <div className="animate-element animate-delay-500 flex items-center justify-between text-sm">
                ...
                e.preventDefault();
                onResetPassword?.();
              }} className="hover:underline text-violet-400 transition-colors">Recuperar palavra-passe</a>
              </div>
            ...
            <p className="animate-element animate-delay-900 text-center text-sm text-muted-foreground">
              Novo na nossa plataforma? <a href="#" onClick={e => {
              e.preventDefault();
              onCreateAccount?.();
            }} className="text-violet-400 hover:underline transition-colors">Criar Conta</a>
            </p>
          </div>
        </div>
      </section>
