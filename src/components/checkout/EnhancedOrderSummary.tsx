...
export const EnhancedOrderSummary = ({ selectedPackage, userCredits = 0 }: EnhancedOrderSummaryProps) => {
  ...
  const activeDiscount = getActiveDiscountForPackage(selectedPackage.id);
  const originalPrice = selectedPackage.price_kwanza;
  const finalPrice = calculateDiscountedPrice(originalPrice, activeDiscount);
  ...
  const pricePerSMS = finalPrice / selectedPackage.credits / 100;
    ...
    <Card className="card-futuristic">
      ...
      <CardContent className="space-y-6">
        ...
        >
          <div className="flex justify-between text-sm">
            <span>Subtotal:</span>
            <span>{(originalPrice / 1000).toFixed(0)}.000 Kz</span>
          </div>
          <div className="flex justify-between text-sm">
          ...
          {activeDiscount && (
            <div className="flex justify-between text-sm">
              <span>
                Desconto {activeDiscount.discount_type === 'percentage' 
                  ? `(${activeDiscount.discount_percentage}%)` 
                  : '(Valor fixo)'}:
              </span>
              <span className="text-green-500">
                -{(discountAmount / 1000).toFixed(0)}.000 Kz
              </span>
            </div>
          )}
          <Separator />
          <div className="flex justify-between text-lg font-bold">
            ...
            <span className="gradient-text text-xl">
              {(finalPrice / 1000).toFixed(0)}.000 Kz
            </span>
          </div>
        </motion.div>

        {/* Economy Highlight - Only show if there's an active discount */}
        {activeDiscount && (
          <motion.div 
          ...
          >
            ...
            <div className="text-sm">
              <p className="font-medium text-green-500">
                Economia de {discountPercentage.toFixed(0)}%
              </p>
              <p className="text-muted-foreground">
                {activeDiscount.description || "Oferta especial por tempo limitado"}
                {activeDiscount.valid_until && (
                  <span className="block text-xs mt-1">
                    Válido até {new Date(activeDiscount.valid_until).toLocaleDateString('pt-BR')}
                  </span>
                )}
              </p>
            </div>
