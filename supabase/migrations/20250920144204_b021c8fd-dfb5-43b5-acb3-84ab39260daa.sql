-- Enhanced Security Fixes for Critical Vulnerabilities (Final)
...
-- 6. Enhanced security monitoring function for sensitive data operations
...
BEGIN
  ...
  IF TG_TABLE_NAME IN ('contacts', 'profiles', 'transactions', 'campaign_targets', 'sms_logs') THEN
    -- Validate session for all sensitive data operations
...
-- Apply enhanced validation trigger to sensitive tables (only for INSERT, UPDATE, DELETE)
...
CREATE TRIGGER enhanced_security_validation_campaign_targets
  BEFORE INSERT OR UPDATE OR DELETE ON public.campaign_targets
  FOR EACH ROW EXECUTE FUNCTION enhanced_data_access_validation();

CREATE TRIGGER enhanced_security_validation_sms_logs
  BEFORE INSERT OR UPDATE OR DELETE ON public.sms_logs
  FOR EACH ROW EXECUTE FUNCTION enhanced_data_access_validation();

-- 7. Create security monitoring function for admins
CREATE OR REPLACE FUNCTION public.get_security_monitoring_summary()
RETURNS TABLE (
...
BEGIN
  -- Only allow admins to access security monitoring
  IF NOT has_role(auth.uid(), 'admin'::app_role) THEN
    RAISE EXCEPTION 'Unauthorized: Only admins can access security monitoring';
  END IF;
  ...
  WHERE created_at > now() - INTERVAL '24 hours'
    AND admin_audit_logs.action LIKE '%security%'
  GROUP BY DATE_TRUNC('hour', created_at), admin_audit_logs.action
  ORDER BY hour DESC;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;