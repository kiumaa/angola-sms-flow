-- Safely restructure credit packages without breaking foreign key constraints
BEGIN;

-- Deactivate all existing packages (preserve for transaction history)
UPDATE public.credit_packages SET is_active = false WHERE is_active = true;

-- Insert the three new packages with final prices (including 12% service fee)
