...
-- 6. Enhanced security monitoring function for non-SELECT operations only
...
BEGIN
  ...
  IF TG_TABLE_NAME IN ('contacts', 'profiles', 'transactions', 'campaign_targets', 'sms_logs') THEN
    -- Validate session for all sensitive data operations (not SELECT)
    IF TG_OP != 'SELECT' AND NOT validate_user_session() AND current_setting('role') != 'service_role' THEN
